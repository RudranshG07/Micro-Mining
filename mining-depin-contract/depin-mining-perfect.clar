...
;; Map to track device IDs globally to prevent duplicates across all wallets
...
(define-public (register-device (device-type (string-ascii 32)) (device-id (string-ascii 64)) (btc-address (string-ascii 64)))
    (let ((miner-address tx-sender))
        ;; Validation - check if this device ID is already registered by anyone
        (asserts! (is-none (map-get? device-registry { device-id: device-id })) ERR-ALREADY-REGISTERED)
        ...
        (map-set device-registry
            { device-id: device-id }
            { owner: miner-address, registered-at: burn-block-height }
        )
        
        ;; For backward compatibility, we'll still use the miners map but update the device info
        ;; This allows wallets to have their "primary" or "latest" registered device
        (map-set miners 
...
;; Start new mining block (admin only)
(define-public (start-block-mining)
    (begin
        (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
        (asserts! (is-eq (var-get mining-active) false) ERR-MINING-ALREADY-ACTIVE)
        
        (let ((new-block burn-block-height))
...
;; Finalize mining round and select winner (admin only)
(define-public (finalize-block-winner)
    (let 
        (
            (current-block (var-get current-mining-block))
            (blocks-since-start (- burn-block-height current-block))
        )
        ;; Validation
        (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
        (asserts! (var-get mining-active) ERR-NO-ACTIVE-MINING)
    ...
    (let
        (
            ...
            (blocks-since-last (if (> last-mining-block u0) 
                                  (- current-burn-block last-mining-block) 
        ...
        (if (and 
            ...
            (begin
                (var-set current-mining-block current-burn-block)
                (var-set mining-active true)
                (map-set block-commitments
                    { block-height: current-burn-block }
                    {
                        miners: (list),
                        total-btc: u0,
                        commitment-count: u0
...
(define-private (select-weighted-winner (target-height uint) (miners-list (list 100 principal)))
    (let ((block-data (unwrap-panic (map-get? block-commitments { block-height: target-height }))))
        (if (> (len miners-list) u0)
            ...
            (let ((selection-seed (mod (+ burn-block-height target-height) u1000000)))
...
(define-read-only (get-mining-stats)
    (let
        (
            ...
            (blocks-since-start (if (> mining-block u0) (- current-burn-block mining-block) u0))
        )
        {
            ;; Current state
            total-miners: (var-get total-registered-miners),
            current-mining-block: mining-block,
            current-bitcoin-block: current-burn-block,
